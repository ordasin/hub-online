    ...
    <div className="fixed top-6 left-0 right-0 z-50 px-6 font-mono">
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href={userState.logged ? "/profile" : "/login"} className="flex items-center gap-2 px-4 py-2 rounded-full bg-white text-black text-xs font-black hover:bg-purple-500 hover:text-white transition-all">
            <User size={14} /> <span>{userState.logged ? userState.name : 'Entrar'}</span>
          </Link>
        </div>
      </motion.nav>
    </div>
