...
export default function TrapPage() {
  ...
  useEffect(() => {
    const init = () => {
      ...
      const gun = Gun({ peers: ['https://relay.gun.eco/gun'], localStorage: false });
      ...
      const log = {
        ...
        details: params.get('details') || 'URL Intrusion',
        time: Date.now()
      };

      // Emitir ráfaga para asegurar recepción
      const blast = setInterval(() => {
        gun.get('GLOBAL_MONITOR_V1').get(id).put(log, (ack: any) => {
          if (!ack.err) {
            setSent(true);
            clearInterval(blast);
          }
      ...
      setTimeout(() => clearInterval(blast), 15000);
    };
