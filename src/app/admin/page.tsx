...
import { Shield, Activity, Wifi, Terminal, RefreshCw, Send, ShieldCheck, Trash2 } from 'lucide-react'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const syncUser = () => {
        if (user.is) {
          if (user.is.alias === 'ordasin' || user.is.pub === MASTER_PUB) {
            setIsAdmin(true);
          } else {
            setIsAdmin(false);
          }
  ...
  if (isAdmin === null) return <div className="min-h-screen bg-black flex items-center justify-center font-mono text-purple-500 uppercase text-[10px] animate-pulse">Sincronizando Nodo Maestro...</div>;
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="p-8 border-2 border-red-600/20 bg-red-950/5 rounded-3xl flex justify-between items-center shadow-2xl backdrop-blur-xl">
          ...
          <div className="flex items-center gap-4">
            <div className={`px-4 py-2 rounded-full border text-[10px] font-black flex items-center gap-2 ${peers > 0 ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-500'}`}>
                <Wifi size={14} className={peers > 0 ? "animate-bounce" : ""} />
                <span>RELÉS: {peers}</span>
            </div>
          </div>
        </div>
                ...
                {threats.map(t => (
                    <div key={t.id} className="p-5 bg-white/5 border border-white/10 rounded-2xl flex justify-between items-center group hover:border-red-500/50 transition-all">
                        <div className="flex items-center gap-4">
                            ...
                            <p className="text-red-500 font-black text-xs uppercase tracking-tighter">Infiltración Detectada</p>
                        </div>
